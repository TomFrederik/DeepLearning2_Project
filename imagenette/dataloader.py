...
class ImagenetteVanilla(Dataset) :

    def __init__(self, train=True):
        super(ImagenetteVanilla, self).__init__()
        root = join('.', 'imagenette', 'data')
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])

        # Transforms
        if train:
            ims_path = join(root, 'imagenette2', 'train')
            t_list = [transforms.RandomResizedCrop(224), transforms.RandomHorizontalFlip()]
        else:
            ims_path = join(root, 'imagenette2', 'val')
    ...
    def get_data(p):
        ...
        for i, sub in enumerate(subdirs):
            im = sorted(glob(sub + '/*'))
            l = np.ones(len(im))*[0,217,482,491,497,566,569,571,574,701][i]
            ims.append(im), labels.append(l)
        return np.concatenate(ims), np.concatenate(labels).astype(np.int64)
...
class ImagenetteCounterfactual(Dataset):
    '''
    args:
    ...
    def __init__(self, ims_path, train=True, n_data=None, mode='silhouette'):
        super(ImagenetteCounterfactual, self).__init__()
    ...
    def get_data(p, train , mode):
        subdirs = glob(p + '/train_cf*') if train else glob(p + '/val_cf*')
        dfs = []
        for sub in subdirs:
...
class CueConflict(Dataset):
    def __init__(self, t_list=[transforms.Resize(256),
        ...
        path = 'imagenette/data/cue_conflict/'
...
def get_imagenette_dls(distributed, batch_size, workers):
    # dataset
    train_dataset = ImagenetteVanilla(train=True)
    val_dataset = ImagenetteVanilla(train=False)
...
def get_cf_imagenette_dls(path, cf_ratio, len_dl_train, distributed, batch_size, workers):
    # determine how many images to use, based on given ratio
    cf_batch_sz = int(cf_ratio * batch_size)
    n_data = cf_batch_sz * len_dl_train

    # dataset
    cf_train_dataset = ImagenetteCounterfactual(path, train=True, n_data=n_data)
    cf_val_dataset = ImagenetteCounterfactual(path, train=False)
